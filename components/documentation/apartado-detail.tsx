...
export default function ApartadoDetail({
  ...
  const canActOnStatus = apartado.status === "enviado";
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start gap-3">
        ...
        {mode === "admin" && canManage && onRemoveApartado && (
          <button
            onClick={async () => {
              if (!confirm("¿Quitar este apartado del cliente?")) return;
              try { await onRemoveApartado(); }
      ...
      <ApartadoFiles
        ...
        canUpload={mode === "client" || (mode === "admin" && !!canManage)}
        canDeleteOwn={mode === "client"}
      ...
      {/* Validar / Rechazar (solo admin con permiso y status="enviado") */}
      {mode === "admin" && canValidate && (
        <div className="border border-gray-100 rounded-xl bg-gray-50/50 px-4 py-3 space-y-2">
          {!canActOnStatus ? (
            <p className="text-xs text-text-muted">
              {apartado.status === "pendiente" && "Esperando que el cliente adjunte documentación."}
              {apartado.status === "validado" && "Este apartado ya ha sido validado."}
              {apartado.status === "rechazado" && "El cliente debe volver a enviar documentación tras el rechazo."}
            </p>
          ) : (
            <>
              <p className="text-[10px] font-semibold uppercase tracking-wider text-text-muted">Acción</p>
              <div className="flex items-center gap-2 flex-wrap">
                <button
                ...
                {!rejecting ? (
                  <button
                    ...
                    disabled={submitting}
                    className="text-xs bg-red-500 text-white px-3 py-1.5 rounded-lg hover:bg-red-600 disabled:opacity-50 cursor-pointer"
                  >
                    Rechazar
                  </button>
                ) : (
                  <div className="flex-1 min-w-[240px] flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 text-xs border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-red-500/30"
                    />
                    <button
                    ...
                    <button
                      ...
                      className="text-xs text-text-muted px-2 cursor-pointer"
                    >
                      Cancelar
                    </button>
                  </div>
