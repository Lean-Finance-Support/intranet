...
// Tipos mínimos para Google Identity Services
declare global {
  interface Window {
    google?: {
      accounts: {
...
export default function ClientLoginPage() {
  ...
  // Handler cuando GIS devuelve el ID token de Google
  async function handleGISCredential({ credential }: { credential: string }) {
    ...
    if (error) {
      setError("No tienes acceso a la plataforma. Contacta con tu asesor.");
      setLoading(false);
      return;
    }
    // Verificar perfil y rol en el servidor
    router.push("/auth/verify");
  }

  // Fallback: OAuth redirect clásico (muestra URL de Supabase brevemente)
